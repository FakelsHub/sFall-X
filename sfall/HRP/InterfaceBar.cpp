...
long IFaceBar::display_width = 0; // ширина области для вывода текста
...
static class Panels {
	...
	void LoadFRMImage(char* name, long winId) {
		...
		if (win->wRect.left <= 0) scr += (frm->frames->width - win->width); // устанавливаем позицию к правой стороне
...
// Масштабирует изображение интерфейса к ширине размера IFACE_BAR_WIDTH
// исключение: первые 30 пикселей и хвост шириной в 460 пикселей не масштабируются
static void InterfaceArtScale(BYTE* scr, long w, long h, BYTE* dst, long dh) {
	// копируем начало шириной в 30 пикселей
	fo::func::buf_to_buf(scr, 30, dh, w, dst, IFaceBar::IFACE_BAR_WIDTH);

	// копируем конец шириной в 460 пикселей
	long sxOffset = w - 460;                         // смещение откуда брать хвост
	long dxOffset = IFaceBar::IFACE_BAR_WIDTH - 460; // смещение куда вставлять хвост

	fo::func::buf_to_buf(scr + sxOffset, 460, dh, w, dst + dxOffset, IFaceBar::IFACE_BAR_WIDTH);

	// устанавливаем значения ширины прямоугольника для источника и назначения
	long dw = dxOffset - 29;
	long sw = sxOffset - 30;

	// маштабируем оставшийся прямоугольник дисплейной части
...
static long __cdecl InterfaceArt(BYTE* scr, long w, long h, long srcWidth, BYTE* dst, long dstWidth) {
	if (HRP::ScreenWidth() >= IFaceBar::IFACE_BAR_WIDTH) {
		...
		// требуемого файла нет, используем дефолтный предоставляемый HRP
		...
		if (frm) {
			// масштабирование интерфейса шириной 800 пикселей до требуемой ширины IFACE_BAR_WIDTH
			InterfaceArtScale(frm->frames->indexBuff, frm->frames->width, frm->frames->height, dst, h);
			delete frm;
		} else {
			// масштабирование ванильного интерфейса шириной 640 пикселей (640-460=180)
...
static void __cdecl display_init_hook_buf_to_buf(BYTE* scr, long w, long h, long srcWidth, BYTE* dispBuff, long dstWidth) {
	...
	// сохраняем в буфер (dispBuff) задний фон интерфейса для перерисовки
