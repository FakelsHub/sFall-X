...
static void __cdecl main_menu_create_hook_buf_to_buf(BYTE* src, long w, long h, long srcW, BYTE* dst, long dstW) {
	...
	fo::Window* win = fo::var::window[fo::var::main_window]; // размер окна всегда равен отмаштабируемого изображения
	h = win->height;
	w = win->width;
	dstW = w;
	...
	/*} else if (MainMenuScreen::MAIN_MENU_SIZE == 2) {
		Image::Scale(src, sw, sh, dst, HRP::ScreenWidth(), HRP::ScreenHeight());*/ // маштабируем до размера установленного разрешения игры
	} else {
		fo::func::buf_to_buf(src, sw, sh, sw, dst, dstW); // direct copy
	}

	// изображение заднего фона для кнопок
	if (btnBackgroundFrm) {
