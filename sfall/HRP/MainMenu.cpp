...
static void __cdecl main_menu_create_hook_buf_to_buf(BYTE* src, long sw, long sh, long srcW, BYTE* dst, long dstW) {
	...
	if (btnBackgroundFrm) {
		...
		x += (long)(sfall::MainMenu::mXOffset * scaleWidth) + offsetX;
		y += (long)(sfall::MainMenu::mYOffset * scaleHeight) + offsetY;
...
static long __fastcall main_menu_create_hook_add_win(long h, long y, long color, long flags) {
	...
	sfall::Graphics::BackgroundClearColor(0);
	...
	sfall::MainMenu::mTextOffset = offsetX;
	if (offsetY) sfall::MainMenu::mTextOffset += offsetY * mainmenuWidth;

	// button text offset
	if (sfall::MainMenu::mXOffset) sfall::MainMenu::mTextOffset += (long)(sfall::MainMenu::mXOffset * scaleWidth);
	if (sfall::MainMenu::mYOffset) sfall::MainMenu::mTextOffset += (long)(sfall::MainMenu::mYOffset * scaleHeight) * mainmenuWidth;
...
static void __fastcall TextScale(long xPos, const char* text, long yPos, long color) {
	...
	yPos *= mainmenuWidth;
	yPos += sfall::MainMenu::mTextOffset;
...
static void __declspec(naked) main_menu_create_hook_text_to_buf() {
	__asm { // eax:xOffset (0), ebp:yPos, edx:text, ebx:txtWidth (640-txtWidth)-1
		...
		add  eax, sfall::MainMenu::mTextOffset;
		add  eax, ebp; // + yPos
...
static long __fastcall ButtonPosition(long &width, long xPos, BYTE* &upImageData, BYTE* &downImageData, long &yPos) {
	...
	/*** Button position ***/
	xPos += (long)(sfall::MainMenu::mXOffset * scaleWidth) + offsetX;
	yPos += (long)(sfall::MainMenu::mYOffset * scaleHeight) + offsetY;
...
void MainMenuScreen::init() {
	...
	if (SCALE_BUTTONS_AND_TEXT_MENU) sf::SafeWrite32(0x4818A9, (DWORD)&buttonImageData); // main_menu_create_
