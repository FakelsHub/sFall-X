...
static long __fastcall main_menu_create_hook_add_win(long h, long y, long color, long flags) {
	...
	scaleXFactor = Setting::ScreenWidth() / (float)640; // TODO: узнать как в mash-HRP происходит получение коэффициента для смещения кнопок для разных разрешениях игры

	sfall::MainMenu::mTextOffset = sfall::MainMenu::mXOffset;
	if (sfall::MainMenu::mYOffset) sfall::MainMenu::mTextOffset += (sfall::MainMenu::mYOffset * w);
...
static void __fastcall TextScale(long xOffset, const char* text, long yPos, long color) {
	...
	yPos *= mainmenuWidth;
	yPos += sfall::MainMenu::mTextOffset; // todo check
