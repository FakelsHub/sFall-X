...
void EdgeClipping::init() {
	...
	// Prevents the rendering of "post roof" objects (eliminates the bug of red pixels remaining from the hex cursor on the edges of the cropped part of the map)
	sf::HookCall(0x489802, obj_render_post_roof_hook_rect_inside_bound);

	// Limiting the creation of a drop-down context menu beyond the visibility of the map or the bottom panel of the interface
	// (this should be removed after changing the context menu implementation)
