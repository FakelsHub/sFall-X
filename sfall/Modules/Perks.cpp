...
static long __stdcall LevelUp() {
	...
	if (!eachLevel) {
		if (!Perks::TraitIsDisabled(TRAIT_skilled) && fo::func::trait_level(TRAIT_skilled)) { // Check if the player has the skilled trait
			eachLevel = 4;
		} else {
			eachLevel = 3;
		}
...
bool Perks::TraitIsDisabled(int traitID) {
