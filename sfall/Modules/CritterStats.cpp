 ...
 *		The module is designed to create additional inherited critter prototypes of their base prototype in memory (with the exception of player and party member prototypes).
 *		The inherited prototype is written a chanded value of stat that is changed by the engine or script script functions.
 *		Stats changed by the set_critter_base_stat and set_critter_extra_stat functions are not saved and resets when the player leaves the location map.
 *		Stats that can be changed using the engine, for example, when using drugs items, armor equipment, or set by the set_critter_stat function is saved
 *		to a file and always affects the critter, except for stats use of drugs, the effects of which are reset when the player leaves the location map (engine behavior).
 *
 *	Краткое описание работы данного кода в модуле:
 *		Модуль предназначен для создания в памяти дополнительных унаследованных прототипов криттеров от их базового прототипа (за исключением прототипов игрока и членов партии).
 *		В унаследованный прототип записывается изменяемая характеристика, которая меняется посредством движка или скритовыми функциями скрипта.
 *		Статистика изменяемая функциями set_critter_base_stat и set_critter_extra_stat не сохраняется и сбрасывается, когда игрок покидает карту локации.
 *		Статистика изменяемая посредством движка, например при применении предметов химии, экипировки брони криттером или установка функцией set_critter_stat сохраняется в файл,
 *		и действует на криттера всегда, кроме измененной статистики при применения химии, эффекты от которой сбрасываются когда игрок покидает карту локации (движковое поведение).
 */
...
// if the set amount value is equal to defVal, this stat is deleted and an attempt to remove the inherited prototype if the individual critter prototype does not have any more changed stats
...
// changes the bonus value of stat (for example, with the adjust_ac_ or perform_drug_effect_ functions when applying drugs items)
// to an individual savedable critter prototype
...
// changes the base stat value with the set_critter_stat script function in the individual saved critter prototype
...
// getting critter stat value using any engine method (for example, the get_critter_stat script function) from the base or individual critter prototype
...
// set_proto_data: sets value parametr to base(regular) object prototype, and to all individual(inherited) critters prototypes with the same PID
...
// get_critter_base_stat/get_critter_extra_stat: gets stats value from an individual's prototype (inherited from the base prototype)
...
// set_critter_base_stat/set_critter_extra_stat: changes the value of stat in the individual prototype of the critter inherited from the base prototype
// the changed value will not be saved when saving the game and will be resets when the player leave the location map
...
// Removes all the inherited prototypes at the leave the map
