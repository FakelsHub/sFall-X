...
void Stats::init() {
	...
	// Allow to change the base stats STAT_unused and STAT_dmg_thresh - STAT_dmg_resist_explosion for set_critter_stat function
	...
	if (!statsFile.empty()) {
		...
		if (!(GetFileAttributes(statFile) == INVALID_FILE_ATTRIBUTES)) { // check exist file
