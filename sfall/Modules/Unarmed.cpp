...
public:
	...
	Hits() {
		...
		hit[Types::palm_strike].costAP = 6;
		...
		hit[Types::piercing_strike].costAP = 8;
		...
		hit[Types::strong_kick].costAP = 4;
		// sec
		...
		hit[Types::piercing_kick].costAP = 9;
...
long Unarmed::GetHitCostAP(fo::AttackType hit) {
	...
	if (!fileUnarmed.empty()) {
		...
		if (!(GetFileAttributes(file) == INVALID_FILE_ATTRIBUTES)) { // check exist file
			...
			for (size_t i = 0; i < Hits::count; _itoa(++i, sHit, 10))
			{
				...
				val = IniReader::GetInt(sHit, "CostAP", -1, file);
				if (val >= 0) hit.costAP = val;
