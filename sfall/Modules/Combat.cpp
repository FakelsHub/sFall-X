...
// Compares the cost (required count of rounds) for a one ammo and the count of current ammo to make an attack or other check
long __fastcall Combat::check_item_ammo_cost(fo::GameObject* weapon, fo::AttackType hitMode) {
	...
	if (checkWeaponAmmoCost == false || currAmmo <= 0) return currAmmo;

	long rounds = 1; // default ammo for single shoot
	...
	if (HookScripts::IsInjectHook(HOOK_AMMOCOST)) {
		AmmoCostHook_Script(1, weapon, newRounds); // в newRounds возвращается новое значение "ammo" умноженное на стоимость
	}
	else if (rounds == 1) { // NOTE: it doesn't make sense to call the item_w_compute_ammo_cost for rounds greater than one
...
static long __fastcall divide_burst_rounds_by_ammo_cost(long currAmmo, fo::GameObject* weapon, long burstRounds) {
	...
	if (HookScripts::IsInjectHook(HOOK_AMMOCOST)) {
		roundsCost = burstRounds;                   // rounds in burst (количество патронов расходуемые в очереди)
		AmmoCostHook_Script(2, weapon, roundsCost); // в roundsCost возвращаетя стоимость!!!
	}

	long cost = burstRounds * roundsCost; // so much ammo is required for this burst (количество патронов в очереди умноженное на 1 или на значение возвращаемое из HOOK_AMMOCOST)
