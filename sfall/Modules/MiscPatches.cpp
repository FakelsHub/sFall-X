...
// When moving to another map of location (that uses the same music) the music playback will not restart from the beginning
...
void MiscPatches::init() {
	...
	if (IniReader::GetConfigInt("System", "SingleCore", 1)) {
		...
		if (sysInfo.dwNumberOfProcessors >= 2) {
			dlog("Applying single core tweak.", DL_INIT);
			HANDLE process = GetCurrentProcess();
			SetProcessAffinityMask(process, 2); // use only CPU ID 1
			CloseHandle(process);
			dlogr(" Done", DL_INIT);
		}
	}
	...
	if (HRP::Setting::ExternalEnabled() == false) SafeWrite32(0x48134D, 38 - (640 * 3)); // main_death_scene_ (shift y-offset 2px up, w/o HRP)
	if (HRP::Setting::ExternalEnabled() == false || HRP::Setting::VersionIsValid) SafeWrite8(0x481345, 4); // main_death_scene_
