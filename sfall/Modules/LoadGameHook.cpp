...
void GetSavePath(char* buf, char* ftype) {
	sprintf(buf, "%s\\savegame\\slot%.2d\\sfall%s.sav", fo::var::patches, ExtraSaveSlots::GetSaveSlot(), ftype); //add SuperSave Page offset
}
...
static void __declspec(naked) LootContainerHook_End() {
	__asm {
		...
		jz   skip; // container is not was created
