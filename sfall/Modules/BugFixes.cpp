...
static void __fastcall CheckAddiction(fo::GameObject* critter) {
	...
	for (size_t i = 0; i < 9; i++)
	{
		...
		if (queue && queue->wait == 0) { // Addiction is not active yet?
...
void BugFixes::init()
{
	...
	//if (GetConfigInt("Misc", "NPCDrugAddictionFix", 1)) {
		...
		HookCalls(item_d_take_drug_hook, { 0x479FBC, 0x47A0AE }); // replaces the item_d_check_addict_ functions with sfall implementation
