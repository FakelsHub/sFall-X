...
// TODO: Add support DirectSound for wav format
...
static void __declspec(naked) soundLoad_hook_A() {
	...
	__asm {
		...
		retn; // exit for SFX sounds effects for which the volume must be set relative from the player's location (see gsound_compute_relative_volume_)
skip:
		mov  acmSoundData, 0;
		mov  esi, [esp + 16 + 4];
...
static long __fastcall SetVolumeAndPan(long volume, fo::ACMSoundData* sound) {
	...
	if (relativeObject->elevation != fo::var::obj_dude->elevation) volume = (32767 / 4); // set 1/4 volume
...
void __stdcall sf_doTimerEvent(UINT uTimerID, UINT uMsg, DWORD_PTR dwUser, DWORD_PTR dw1, DWORD_PTR dw2) {
...
static void __declspec(naked) fadeSounds_hook() {
	__asm {
		...
		or   word ptr [eax + 0x40], 0x10; // state set 'AfterFade' flag
...
static void __declspec(naked) soundContinue_hack() {
	__asm {
		mov  dx, [ebx + 0x40];
		test dx, 0x10; // check state 'AfterFade' flag (fix flag set in fadeSounds_hook)
		jz   skip;
		cmp  fadeProcess, 1;
		je   skip;
...
void Sound::init() {
	...
	// Enables support for panning sound for sfx effects and reduce the sound volume for an object located on a different elevation of the map
