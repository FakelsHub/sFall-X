...
// The hook is executed twice when entering the barter or after transaction, the first time for the player, the second time for the trader
static DWORD __fastcall BarterPriceHook_Script(register fo::GameObject* source, register fo::GameObject* target, DWORD callAddr) {
	...
	args[8] = (DWORD)(callAddr == 0x474D51); // pressed offers button
