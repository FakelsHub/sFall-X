	...
	if (create) {
		...
		if (FAILED(d3d9->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, window, D3DCREATE_HARDWARE_VERTEXPROCESSING | D3DCREATE_FPU_PRESERVE| mThreadFlags, &params, &d3d9Device))) { // D3DCREATE_PUREDEVICE
			...
			if (params.Windowed) {
				...
				dlog(" Failed to create hardware vertex processing device. Using software vertex processing instead.", DL_MAIN);
			}
		}
		...
		bool A8IsSupport = Graphics::GPUBlt && (d3d9Device->CreateTexture(ResWidth, ResHeight, 1, 0, D3DFMT_A8, D3DPOOL_SYSTEMMEM, &mainTex, 0) == D3D_OK);

		if (Graphics::GPUBlt) {
			const BYTE* shader = (A8IsSupport) ? gpuEffectA8 : gpuEffectL8;
			const UINT size = (A8IsSupport) ? sizeof(gpuEffectA8) : sizeof(gpuEffectL8);
			if (D3DXCreateEffect(d3d9Device, shader, size, 0, 0, 0, 0, &gpuBltEffect, 0) == D3D_OK) {
				...
				textureFormat = (A8IsSupport) ? D3DFMT_A8 : D3DFMT_L8; // D3DFMT_A8 - not supported on some older video cards
			} else {
				MessageBoxA(window, "Failed to create shader effects.\nSwithing to CPU for conversion the palette.", "SFALL DirectX 9", MB_TASKMODAL | MB_ICONWARNING);

				if (mainTex) SAFERELEASE(mainTex); // release texture D3DFMT_A8 format
				Graphics::GPUBlt = 0;
				A8IsSupport = false;
			}
		}

		if (!A8IsSupport && d3d9Device->CreateTexture(ResWidth, ResHeight, 1, 0, textureFormat, D3DPOOL_SYSTEMMEM, &mainTex, 0) != D3D_OK) {
			...
			MessageBoxA(window, "Texture format error.\nGPU does not support the D3DFMT_L8 texture format.\nNow CPU is used to convert the palette."
								"Set 'GPUBlt' option to CPU for hide this warning message.",
								"SFALL DirectX 9", MB_TASKMODAL | MB_ICONWARNING);
...
public:
	...
	FakeDirectDrawSurface(bool primary, DDSURFACEDESC* desc) {
		...
		if (primary) {
			...
			// for enabled GPUBlt used the mainTex texture as source buffer
		} else {
	...
	*/
	HRESULT __stdcall Lock(LPRECT a, LPDDSURFACEDESC b, DWORD c, HANDLE d) {
		...
		if (isPrimary) {
			...
			mveDesc.lPitch = m_width;    //*(DWORD*)FO_VAR_lastMovieW;
			mveDesc.dwHeight = m_height; //*(DWORD*)FO_VAR_lastMovieH;
			*b = mveDesc;
			b->lpSurface = lockTarget;
	...
	/* Called from:
		...
		004F5EFA/0x4F5F0B nf_mve_buf_unlock (from MVE_rmStepMovie_)
	*/
...
#define _DLL_NAME "d3dx9_43.dll"
		...
		if (!h) {
			...
			MessageBoxA(0, "You have selected DirectX graphics mode, but " _DLL_NAME " is missing.\n"
						   "Switch back to DirectDraw (Mode=0), or install an up to date version of DirectX 9c.", 0, MB_TASKMODAL | MB_ICONERROR);
#undef _DLL_NAME
			ExitProcess(-1);
			return;
		}
		...
		// Replaces the screenshot saving implementation for sfall DirectX 9
		...
		if (HRP::Setting::VersionIsValid) {
			...
			// Patch HRP to fix the issue of displaying a palette color with an index of 255 for images (splash, end slides)
	...
	if (HRP::Setting::IsEnabled()) {
		...
		// Reassigning the WindowProc function to avoid an unnecessary jump from the engine code
...
HRESULT __stdcall SaveScreenA() {
	return (HRESULT)sfall::SaveScreen("CrashScreen.png");
}
