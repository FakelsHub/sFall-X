...
struct GlobalScript {
	...
	int isLoading; // used for the return value for the game_loaded function
...
uint32_t availableGlobalScriptTypes = 0;
static uint32_t isGlobalScriptLoading = 0;
static bool gameIsResets;
...
static void __stdcall FreeProgram(fo::Program* progPtr) {
	if (gameIsResets || (sfallProgsMap.find(progPtr) == sfallProgsMap.cend())) { // only delete non-sfall scripts or when actually loading the game
		__asm {
			mov  eax, progPtr;
...
bool IsGameScript(const char* filename) {
	...
	return false; // name of script was not found in scripts.lst
}

// loads and initialize script file (for normal game script)
long __fastcall ScriptExtender::InitScript(long sid) {
	fo::ScriptInstance* scriptPrt;
	if (fo::func::scr_ptr(sid, &scriptPrt) == -1) return -1;

	scriptPrt->program = fo::func::loadProgram(fo::var::scriptListInfo[scriptPrt->scriptIdx & 0xFFFFFF].fileName);
	if (!scriptPrt->program) return -1;
	if (scriptPrt->program->flags & 0x124) return 0;

	// fill lookup table
	fo::func::scr_build_lookup_table(scriptPrt);

	scriptPrt->flags |= 4 | 1; // init | loaded
	scriptPrt->action = fo::Scripts::ScriptProc::no_p_proc;
	scriptPrt->scriptOverrides = 0;

	fo::func::runProgram(scriptPrt->program);
	return 0;
}

static void LoadGlobalScriptsList() {
	dlog("Running global scripts...\n", DL_SCRIPT);
...
int __stdcall ScriptExtender::ScriptHasLoaded(fo::Program* script) {
	if (lastProgram.script == script) { // for fast
...
static uint32_t TimedEventNextTime() {
	uint32_t nextTime;
	__asm {
		push ecx;
...
static uint32_t script_chk_timed_events_hook() {
