...
namespace WinFlags {
	enum WinButtonFlags : long
	{
		...
		MoveOnTop         = 0x000004, // places the window at the top when it is created
		Hidden            = 0x000008,
		Exclusive         = 0x000010,
...
namespace AIpref {
	enum distance : long
	{
		stay_close            = 0, // держаться рядом, атакующий будет держаться на дистанции не превышающей 5 гексов от игрока (только для нарников, определено в ai_move_steps_closer, cai_perform_distance_prefs)
		charge                = 1, // атакующее поведение, AI будет пытаться всегда приблизиться к своей цели перед атакой или после атаки
		snipe                 = 2, // атака с расстояния, при сокращении дистанции между атакующим и целью, атакующий попытается отойти от цели на дистанцию до 10 гексов
		on_your_own           = 3, // специального поведение для этого не определено
		stay                  = 4  // ограниченное передвижение в бою, атакующий будет по возможности оставаться на том гексе где начал бой (определено в ai_move_steps_closer, ai_move_away)
	};

	// установленные пресеты для напарников
	...
	{
		whomever_attacking_me = 0, // атаковать цель которую атакует игрок (только для напарников)
		strongest             = 1, // атаковать сильных (в бою будет переключаться на более сильные цели)
		weakest               = 2, // атаковать слабых
		whomever              = 3, // кого угодно
		closest               = 4, // атаковать только ближние цели
	};
