...
struct GameObject {
	...
	union {
		struct {
			...
			ProtoID ammoPid;
			long unused[8]; // offset 0x44 (don't save)
		} item;
		...
		struct {
			...
			long unused[9];             // offset 0x40 (don't save)
		} Scenery;
	};
...
struct Program {
	...
	long field_14;      // unused engine?
	long field_18;      // unused engine?
	long *dStackPtr;
	long *aStackPtr;
	long *dStackOffs;
	long *aStackOffs;
	...
	long field_6C;      // unused engine?
	long field_70;      // unused engine?
	long field_74;      // unused engine?
...
struct AIcap {
	...
	long min_to_hit;
	long min_hp;
	long aggression; // unused (TODO: если значение меньше 50 то случайное попадание не приведет к бою)
	long hurt_too_much;
	long secondary_freq;
	long called_freq;
	long font;
...
struct QueueAddictData {
	long  wait;      // 1 - waiting for the apply of addiction effects (perk is not active yet)
	DWORD drugPid;
